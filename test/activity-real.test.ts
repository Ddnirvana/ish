...
test("animated agent activity changes and interruption returns to zsh", async (t) => {
	...
	const first = tmux(["capture-pane", "-p", "-t", "activity:0.0", "-S", "-20"]).stdout;
	await new Promise((resolve) => setTimeout(resolve, 180));
	const second = tmux(["capture-pane", "-p", "-t", "activity:0.0", "-S", "-20"]).stdout;
	assert.match(first, /Pi is working/);
	assert.match(second, /Pi is working/);
	assert.notEqual(first, second, "activity frame should change while Pi is running");
